...
```mermaid
flowchart TD
    A[Customer initiates payment] --> B[PO routes to Acquirer]
    B --> C[Acquirer authorizes transaction]
    C --> D{Capture Method}
    D -->|Auto Capture| E[Transaction Captured]
    D -->|Manual Capture| E
    E --> F[PO initiates Settlement Process]
    F --> G[Reconcile with internal records]
    G --> H[Deduct MDR and Fees]
    H --> I[Generate Settlement Report]
    I --> J[Transfer Net Amount]
    J --> K[Funds credited to merchant bank account - T+1 to T+3]
