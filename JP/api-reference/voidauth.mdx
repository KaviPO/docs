---
title: '認証の取消'
description: 'Void Authorisation APIは、資金がキャプチャされる前に、既に承認された支払いをキャンセルして、顧客のアカウントに対するホールドを解除するために使用されます。'
---

### APIの詳細:
- HTTPメソッド: `POST`
- エンドポイントURL: `/api/v1/server-to-server-interface/void`

### 重要なポイント
<Info>
- Void Authorisation APIは、承認されたがまだキャプチャされていない取引を進めないことが決定された場合に重要です。
- 承認が取り消されると、顧客のアカウントに対するホールドされていた資金が解除され、再び利用可能になります。
- このAPIは、注文が発送前にキャンセルされた場合や、取引の詳細が変更されて再承認が必要な場合に使用されます。
</Info>
...
| **transaction_type**   | 取引タイプ（例：承認取引のVOID）。                                                    |
