---
title: 'サブスクリプションの一時停止'
description: 'サブスクリプションの一時停止APIを使用すると、マーチャントは顧客のサブスクリプションを一時的に停止し、サブスクリプションが再開されるまで今後の請求サイクルを停止できます。'
---

### APIの詳細
- HTTPメソッド: `POST`
- エンドポイントURL: `/api/v1/server-to-server-interface/subscription/{subscription_id}/pause`

## 主なポイント
<Info>
### サブスクリプションの一時停止
- マーチャントはこのAPIを使用してサブスクリプションを一時停止することができ、サブスクリプションが再開されるまで顧客への今後の請求を停止します。
- 一時停止期間中、サブスクリプションはアクティブな状態にありますが、再開されない限り請求やサービスは発生しません。
### サブスクリプションステータス
- サブスクリプションはシステム内で一時停止のステータスに変更されます。
- サブスクリプションが一時停止されている間、請求サイクルは処理されませんが、マーチャントはサブスクリプションを再開するタイミングを管理できます。
### 必須パラメータ
