---
title: '返金'
description: 'Refund APIは、成功した購入またはキャプチャ取引後に、顧客に資金を返還するために使用されます。'
---

### APIの詳細:
- HTTPメソッド: `POST`
- エンドポイントURL: `/api/v1/server-to-server-interface/refund`


### 重要なポイント
<Info>
- Refund APIは、取引を逆転させて、顧客の支払い方法に資金を返すためにマーチャントが使用できるものです。
- このAPIは、注文のキャンセル、商品の返品、または顧客が請求に異議を唱えた場合などに通常使用されます。
- 返金は、リクエストに指定された金額に応じて部分的または全額で行うことができます。部分返金が行われた場合、元の取引の残りの残高は引き続き有効です。
</Info>
...
| **amount**             | int/string| はい           | 銀行の規定に従う | 返金される金額。これはキャプチャ/購入された金額を超えてはいけません。金額の制限は銀行によります。 |
| **merchant_id**         | string    | はい           | 10            | マーチャントID（DASMID）。                                                           |
| **notes**              | string    | いいえ         | 128           | 返金取引の参照（オプション）。                                                     |
...
| **transaction_type**   | 取引タイプ（例：キャプチャされた取引のREFUND）。                                       |
